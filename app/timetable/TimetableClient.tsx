  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 pb-6 border-b border-border/40">
        <div>
          <h2 className="text-2xl font-heading font-bold tracking-tight text-foreground mb-1">
            Study Schedule
          </h2>
          ...
          <div className="flex gap-2 justify-center border-b border-border/20 pb-4 overflow-x-auto">
            {DAYS.map((day, idx) => {
              ...
              return (
                ...
                >
                  <span className="text-xs font-medium opacity-70 leading-none mb-1">{day.slice(0, 3)}</span>
                  <span className="text-base font-heading font-bold leading-none">{format(date, 'd')}</span>
                </button>
              );
            })}
          </div>
          ...
          <div className="space-y-3.5 pt-2">
            {daysTimelines[activeDay]?.length === 0 ? (
              <p className="text-sm font-medium text-muted-foreground text-center py-6 bg-muted/50 rounded-2xl border border-border/50">
                No classes or tasks scheduled for {activeDay}. Add a study block from Manage Schedule to fill this day.
              </p>
            ) : (
                ...
                return (
                  ...
                  >
                    ...
                    <div className="shrink-0 flex items-center">
                      {isStudy && item.isDone && (
                        <span className="text-xs font-medium text-emerald-600 bg-emerald-500/10 border border-emerald-500/20 px-2.5 py-1 rounded-full flex items-center gap-1">
                          <CheckCircle2 className="w-3 h-3" /> Completed
                        </span>
                      )}
                      {isStudy && item.isMissed && (
                        <span className="text-xs font-medium text-destructive bg-destructive/10 border border-destructive/20 px-2.5 py-1 rounded-full flex items-center gap-1">
                          <XCircle className="w-3 h-3" /> Missed
                        </span>
                      )}
                      {isStudy && !item.isDone && !item.isMissed && (
                        <span className="text-xs font-medium text-primary/80 bg-primary/5 border border-primary/10 px-2.5 py-1 rounded-full">
                          {toTitleCase(item.subType)}
                        </span>
                      )}
                      {isSchool && (
                        <span className="text-xs font-medium text-blue-600/80 bg-blue-500/5 border border-blue-500/10 px-2.5 py-1 rounded-full">
                          Lectures
                        </span>
                      )}
                      {isBreak && (
                        <span className="text-xs font-medium text-muted-foreground bg-muted/20 border border-border/10 px-2.5 py-1 rounded-full">
                          Break
                        </span>
                      )}
                    </div>
          ...
          {DAYS.map((day, dayIdx) => {
            ...
            return (
              <div 
                key={day} 
                className={cn(
                  "bg-card border border-border/60 shadow-sm rounded-2xl p-5 flex flex-col md:flex-row items-stretch md:items-center gap-6 relative overflow-hidden transition-colors duration-200",
                  isToday ? "border-primary" : "hover:border-primary/20"
                )}
              >
                {/* Day block */}
                <div className={cn(
                  ...
                  isToday ? "bg-primary text-primary-foreground border-primary" : "bg-muted/40 border-border/50 text-foreground"
                )}>
                  <span className="text-xs font-medium opacity-70 leading-none">{day.slice(0, 3)}</span>
                  <span className="text-xl font-heading font-bold leading-none">{format(date, 'd')}</span>
                </div>

                {/* Flow of timeline capsules */}
                <div className="flex-1 flex flex-wrap gap-3 items-center">
                  {dayTimeline.length === 0 ? (
                    <span className="text-xs font-medium text-muted-foreground italic px-2">Free day</span>
                  ) : (
                    dayTimeline.map((item, idx) => {
                      ...
                      return (
                        <div
                          key={idx}
                          className={cn(
                            "py-2.5 px-4 rounded-xl border text-xs font-medium transition-colors duration-200 flex flex-col justify-between min-w-[150px] max-w-[200px] shrink-0 relative overflow-hidden",
                            isSchool && "bg-blue-500/5 border-blue-500/20 text-blue-700",
                        ...
                        >
                          ...
                          <span className={cn(
                            "font-semibold truncate leading-tight block",
