...
import {
  Trophy, Clock, X, Target, FileText,
      ...
      <div className="space-y-5">
        <div className="flex items-start justify-between gap-4">
          <div className="min-w-0 space-y-1">
            <p className="text-xs font-medium text-muted-foreground">Weekly report</p>
            <h3 className="font-heading text-xl font-bold text-foreground">{format(new Date(summary.startDate), "d MMM")} to {format(new Date(summary.endDate), "d MMM")}</h3>
        ...
        <dl className="grid grid-cols-3 gap-3">
          {[
            ...
            { icon: CheckCircle2, label: "Blocks", value: `${summary.completedSessions}`, c: "text-success" },
            { icon: CalendarDays, label: "Days", value: `${summary.activeDays}/7`, c: "text-orange-500" },
          ].map(s => (
